...
class CFields {
	 ...
	 * 
	 * */
	//Функция единовразово считывает все поля, чтобы постоянно не обращаться к базе
	...
	static function Show_Edit() {
		...
		if (! self::$flag_edit_out) {
			...
			/*Делаем запрос к базе, чтобы получить айдишники всех опубликованных постов. Потов в цикле выводим кастомные
				 * поля для каждого из поста. $posts_id представляет собой двумерный массив ['индекс_массива'][id]. Далее в зависимости
				 * от типа поля создаем объект того поля и вызываем метод OutField() соответсвующего поля
				 * */
		...
		//echo preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', "", $result);
