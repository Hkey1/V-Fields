...
class Digit extends FieldType {
	...
	function Mysql_Where($pieces = NULL, $param = NULL, $value = NULL) {
		...
		$field_param = array ();
		/*
		 * Парсим $param...Он представлен в виде cf_xxx_xxx....Вытягиваем имя
		 * поля и условие поиска Проверяем существует ли такое имя. Если
		 * существует то в зависимости от условия формируем WHERE
		 */
		if (preg_match ( '/^cf_(.*)_(.*)/i', $param, $field_param )) {
			...
			// Если fieldname в GET строке действительно существует, то
			// применяем фильтры
			...
			/*
			 * Если поле идет без условия..т.е. cf_xxx=4 Допустим то вытягиваем
			 * имя поля...тоже проверяем на наличие такого поля и если
			 * существует то изменяем соответствующее условие поиска WHERE
			 */
		...
		// Это копия wordpress'овского массива $pieces. Он к нам приходит, мы
		// изменяет элемент ['where] и отправляем назад.
		return $pieces;
	}
}
?>