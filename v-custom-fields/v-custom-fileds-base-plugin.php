<?php
/*
Plugin Name: V Custom Fields: base plugin
Plugin URI: http://no-site.org/
Description: Base plugin
Author: Oleg Kovalyov
Version: 1.0
Author URI: http://vkontakte.ru/id4354304
*/
...
function custom_search_where($where) {
    ...
    if (is_search() && isset($_GET['s'])) {
        ...
        //return " AND (CAST($wpdb->postmeta.meta_value AS SIGNED) > 6) AND ($wpdb->posts.post_status = 'publish') ";
        /*INDEX SEARCH*/
...
function intercept_query_clauses( $pieces )
{
	...
	if($_GET['isearch'] == 1)
	{
		...
		{
			//Массив, одним из элементов которого является имя поля(после распарсивания)
			$parts = array();
			//сортировка
			if($key == "cf_order_by")
			{
				...
				foreach($order_parts as $row)
				{
					...
					$order_type = $arr[1];
					/*
					 * Если имя поля и условия сортировки - строки и если такое поле существует и метод сортировки верный,
					 * то в условие ORDER BY ставим то, что нужно...в противном случае забиваем на сортировку и едем дальше
					 * */
			...
			/*
			 * Здесь вытягиваем паресром имя поля, проверяем существует ли такое поле..Если существует, то создаем объект 
			 * это класса и условие поиска уже создает сам объект..Управление переходит к нему....Передаем и принимаем уже готовый 
			 * wordpress'овский массив $pieces...Правим лишь элеменм 'where'.
			 * */
		...
		//Добавляеем условие что ищем тока по полям с включенным индесным поиском
		$pieces['where'].= " AND  ".$wpdb->prefix."v_field_options.isearch = 1";
		//Сопоставляем кастомное поле из wp_postmeta и v_field_options(одна из таблиц нашего плагина)
